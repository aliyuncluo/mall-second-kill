...
import com.cluo.mall.miaosha.dao.StockLogDOMapper;
...
public class ItemServiceImpl implements ItemService {
    ...
    public boolean decreaseStock(Integer itemId, Integer amount) throws BusinessException{
        ...
        if(result > 0){
            //更新库存成功
			Boolean isResult = mqProducer.syncReduceStock(itemId, amount);
			if(!isResult) {
				redisTemplate.opsForValue().increment("promo_item_stock_"+itemId,amount.intValue());
				return false;
			}
            return true;
        }else if(result==0) { //库存已售罄
        	//打上库存已售罄的标识
	...
	public ItemModel getItemByIdInCache(Integer id) {
		...
		if(itemModel==null) {
			...
			redisTemplate.expire("item_validate_"+id, 10, TimeUnit.MINUTES); //10分钟有效期
		}
		return itemModel;
	}
	...
	public boolean asyncDecreaseStock(Integer itemId, Integer amount) {
		...
		Boolean mqResult = mqProducer.syncReduceStock(itemId, amount);
		return mqResult;
	}
